...
LOOK_ID = 8792
CLINIC_HOURS = 9  # 8 AM–5 PM (9 hours)
MINUTES_PER_HOUR = 60
...
def calculate_capacity(df):
    """Total clinic capacity per day = chairs × 9 hours × 60 minutes."""
...
def get_optimal_times(df, location, duration):
    ...
    # Compute next available minute within 8 AM–5 PM
    ...
    def compute_next_available(row):
        ...
        # If next time exceeds 5 PM, exclude it
        if next_time.time() > CLINIC_END:
            return None
        return next_time.strftime("%I:%M %p")  # 12-hour AM/PM format
    ...
    # Sort by earliest date and time
